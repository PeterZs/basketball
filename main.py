...
parser.add_argument("config", type=str,
    help="Configure file used for training. Please refer to files in `cfg` folder.")
...
def train(env, model, ckpt_dir, training_params):
    ...
    GAMMA_LAMBDA = GAMMA * training_params.lambda_
    ...
    while not env.request_quit:
        ...
        buffer["terminate"].append(info["terminate"])
        ...
        if len(buffer["s"]) == HORIZON:
            ...
            if env.discriminators:
                with torch.no_grad():
                    for name, data in buffer_disc.items():
                        ...
                        end_frame_ = end_frame.unsqueeze(1)

                        length = torch.arange(fake.size(1), 
                            dtype=end_frame.dtype, device=end_frame.device
                        ).unsqueeze_(0)         # 1 x L
                        mask = length <= end_frame_     # N x L
                        mask_ = length >= fake.size(1)-1 - end_frame_
            ...
            with torch.no_grad():
                ...
                if multi_critics:
                    ...
                    reward_weights = None
                rewards_disc = [
                    disc(ob, seq_end_frame, normalize=False).clamp_(-1, 1).mean(-1, keepdim=True)
                    for _, disc, _, ob, seq_end_frame in disc_data
                ]
                if has_goal_reward:
                    rewards_task = torch.cat(buffer["r"])
                    if rewards_disc:
                        rewards = torch.cat((*rewards_disc, rewards_task), -1)
                    else:
                        rewards = rewards_task
                else:
                    rewards_task = None
                    if len(rewards_disc) > 1:
                        rewards = torch.cat(rewards_disc, -1)
                    else:
                        rewards = rewards_disc[0]
            ...
            if epoch % LOG_INTERVAL == 1:
                lifetime = info["lifetime"].to(torch.float32).mean().item()
                ...
                if logger is not None:
                    if "success_rate" in info:
                        logger.add_scalar("perf/success_rate", info["success_rate"], epoch)
                    if "shot_percent" in info and info["shot_percent"]:
                        logger.add_scalar("perf/shot_percent", np.mean(info["shot_percent"]), epoch)    
                        info["shot_percent"].clear()
                    if "ball_hold" in info and info["ball_hold"]:
                        logger.add_scalar("perf/ball_hold", np.mean(info["ball_hold"]), epoch)    
                        info["ball_hold"].clear()
                    if "caught_rate" in info and info["caught_rate"]:
                        logger.add_scalar("perf/caught_rate", np.mean(info["caught_rate"]), epoch)    
                        info["caught_rate"].clear()
...
if __name__ == "__main__":
    ...
    value_dim = len(env.disc_dim)+env.rew_dim
    model = ACModel(env.state_dim, env.act_dim, env.goal_dim[0], env.goal_dim[1], value_dim)
