...
class ACModel(torch.nn.Module):
        ...
        def __init__(self, state_dim, goal_dim, value_dim=1, latent_dim=256):
            ...
            if goal_dim:
                self.embed_goal = torch.nn.Sequential(
                    torch.nn.Linear(goal_dim, 1024 if goal_dim > latent_dim else latent_dim),
                    torch.nn.ReLU6(),
                    torch.nn.Linear(1024 if goal_dim > latent_dim else latent_dim, latent_dim)
                )
            i = 0
            for n, p in self.mlp.named_parameters():
                if "bias" in n:
    ...
    def __init__(self, state_dim: int, act_dim: int, actor_goal_dim: int=0, critic_goal_dim: int=0, value_dim: int=1, 
        normalize_value: bool=True,
        init_mu:Optional[Union[torch.Tensor, float]]=None, init_sigma:Optional[Union[torch.Tensor, float]]=None
    ):
        super().__init__()
        assert critic_goal_dim >= actor_goal_dim
        self.state_dim = state_dim
        self.actor_goal_dim = actor_goal_dim
        self.critic_goal_dim = critic_goal_dim
        self.actor = self.Actor(state_dim, act_dim, self.actor_goal_dim, init_mu=init_mu, init_sigma=init_sigma)
        self.critic = self.Critic(state_dim, self.critic_goal_dim, value_dim)
    ...
    def observe(self, obs, norm=True):
        if self.critic_goal_dim > 0:
            s = obs[:, :-self.critic_goal_dim]
            g = obs[:, -self.critic_goal_dim:]
        else:
            s = obs
            g = None
    ...
    def act(self, obs, seq_end_frame, stochastic=None, unnorm=False):
        ...
        pi = self.actor(s, seq_end_frame, g if self.critic_goal_dim == self.actor_goal_dim else g[...,:self.actor_goal_dim])
        if stochastic:
            a = pi.sample()
            lp = pi.log_prob(a)
