...
class ReferenceMotion():
    def __init__(self, motion_file: Union[str, Sequence[str]],
        ...
        if type(motion_file) == str:
            self.motion.extend(self.load_motions(motion_file, skeleton, controllable_links, key_links))
        else:
            for m in motion_file:
                self.motion.extend(self.load_motions(m, skeleton, controllable_links, key_links))
        self.prepare_data()
    ...
    def load_motions(self, motion_file, skeleton, controllable_links, key_links):
        ...
        for f, (w, motion_file) in enumerate(zip(motion_weights, motion_files)):
            ...
            assert os.path.splitext(motion_file)[1] == ".json"
    ...
    def sample(self, n, max_time=None, truncate_time=None):
